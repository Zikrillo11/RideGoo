...
export default function CustomerRatingModal({ orderId, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await api.post('/Ratings/rate-customer', {
        orderId,
        score,
        comment: comment || null,
      });
      toast.success('Baho qoyildi!');
      onSuccess();
      onClose();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-sm p-6 relative">
        ...
        <h3 className="text-lg font-bold text-gray-900 mb-1">Mijozni baholang</h3>
        <p className="text-sm text-gray-500 mb-5">Mijoz bilan safar qanday otdi?</p>
